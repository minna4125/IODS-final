...
#Research questions
#####Can we predict if the anime is popular based on the variables?
...
4.genre_clusters is the number of cluster that the animes genres belong to [1-7].
...
######There was 44 diffrent genres in the data and over 2000 unique combination of these. It is too much variables to be used in this model, but genre I find genre to be one of the key elements in determing if the anime is being liked or not. So I decided to group these 2000 genres in reasonable amount of groups by clustering.
####How the number of clusters was determined? 
######I used kmeans function with nstart ( the number of random sets chosen) as 30, because 10 wasn't enough and set the iterations to 10000 so it was enough too. Then I relied on human inteligence in valuing the number of clusters. I increased the number of clusters untill the 50 sample from each cluster seemed reasonably put in the same category. This method could be improved, but I don't know how to.
```{r, eval=TRUE}
...
```{r, eval=TRUE}
    #Bubble gum fix
    anime <- read.csv("anime_gen")
    anime <- filter(anime, complete.cases(anime))
    library(dplyr)

    #Making the model
    m <- glm(rating ~ episodes+members+genre_cluster, data = anime, family = "binomial")
    summary(m)
